...
void engine::perspective_camera::on_update(const timestep& timestep)
{
	...
	//custom camera
	if (player_health > 50) { //player can fly but limited
		if (m_position.y > 4.f || m_position.y < 2.f)
			m_position.y = y_value;
	}
	else {					 //player is grounded at y=2
		if (m_position.y != 2) {
			y_value = 2.f;
			m_position.y = y_value;
		}
...
void engine::perspective_camera::get_player_health(float val) {
	player_health = val;
}
