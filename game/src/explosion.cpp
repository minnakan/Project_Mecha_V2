...
explosion::explosion(std::string sprite_filename, uint32_t frames_x, uint32_t frames_y, uint32_t num_frames)
{
...
void explosion::on_update(const engine::timestep& time_step)
{
	if (!s_active)
		return;
...
void explosion::on_render(const engine::perspective_camera& camera, engine::ref<engine::shader> shader)
{
	if (!s_active)
		return;
...
void explosion::activate(glm::vec3 position, float width, float height)
{
	s_active = true;
	m_timer = 0.0f;
	m_current_frame = 0;
...
engine::ref<explosion> explosion::create(std::string sprite_filename, uint32_t frame_width, uint32_t frame_height, uint32_t num_frames)
{
	return std::make_shared<explosion>(sprite_filename, frame_width, frame_height, num_frames);
}
